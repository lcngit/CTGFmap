...
%_version/date: v1.0.4r0/2017.09.23
...
dsz = round(dsz, 0);
[train_strcolor, train_numcolor] = CTGFmap_GetColorPaper('Select train paper color: ');
[test_strcolor, test_numcolor] = CTGFmap_GetColorPaper('Select tests paper color: ');
ppcolsuf = strcat('_', train_strcolor, 'x', test_strcolor);

diaryfile = strcat('diffpaper', ppcolsuf ,'full_len_mtd_', num2str(mtd), ...
    '_dsz', num2str(dsz),'_log_', ...
    strrep(strrep(datestr(now), ':', '_'), ' ', '-'), '.txt');
diary(diaryfile);

initime = datetime('now');
disp(['+++ Begin - Processing - ', datestr(initime), ' +++']);
disp('+++ Different Paper Colors Full Length Method Training, Validation and Testing +++');
if dsz > 0
...
for exp_num = 1:n_exp
    ...
    for cls = 1:n_cls
        ...
        disp('*** Selecting documents paper colors for training and testing ***');
        VecsColor = floor(DocIds ./ C_DSETPLAN_COLOR_ID);
        if train_numcolor ~= C_PAPERCOLOR_ALL
            disp(['    Training paper color selected: ', ...
                num2str(train_numcolor)]);
            FiltTrain = VecsColor == train_numcolor;
            TrainExpPlan = ExpPlan(FiltTrain, :);
            TrainFeatVecs = FeatVecs(FiltTrain, :);
            TrainClassIds = ClassIds(FiltTrain, :);
            TrainDocIds = DocIds(FiltTrain, :);
            TrainFrmIds = FrmIds(FiltTrain, :);
        else
            disp('    All paper colors selected for training!');
            TrainExpPlan = ExpPlan;
            TrainFeatVecs = FeatVecs;
            TrainClassIds = ClassIds;
            TrainDocIds = DocIds;
            TrainFrmIds = FrmIds;
        end
        
        if test_numcolor ~= C_PAPERCOLOR_ALL
            disp(['    Tests paper color selected: ', ...
                num2str(test_numcolor)]);
            FiltTest = VecsColor == test_numcolor;
            TestExpPlan = ExpPlan(FiltTest, :);
            TestFeatVecs = FeatVecs(FiltTest, :);
            TestClassIds = ClassIds(FiltTest, :);
            TestDocIds = DocIds(FiltTest, :);
            TestFrmIds = FrmIds(FiltTest, :);
        else
            disp('    All paper colors selected for tests!');
            TestExpPlan = ExpPlan;
            TestFeatVecs = FeatVecs;
            TestClassIds = ClassIds;
            TestDocIds = DocIds;
            TestFrmIds = FrmIds;
        end
        ...
        [ClassModel, elaptrain, n_train] = CTGFmap_ClassModel_Train( ...
            cls_id, mtd, exp_num, TrainExpPlan, TrainFeatVecs, FeatIds, ...
            TrainClassIds, TrainDocIds, TrainFrmIds);
        iel = iel + 1;
        ...
        [~, ~, elapval, n_val] = CTGFmap_ClassModel_Test(cls_id, mtd, ...
            exp_num, C_EXP_VALIDATION, TestExpPlan, TestFeatVecs, ClassModel, ...
            TestClassIds, TestDocIds, TestFrmIds, fsuffix);
        iel = iel + 1;
        ElapsedTime(iel, C_ELAPSED_RANK)     = rnk_type;
        ElapsedTime(iel, C_ELAPSED_CLASS)    = cls_id;
        ElapsedTime(iel, C_ELAPSED_EXPNUM)   = exp_num;
        ...
        [~, ~, elaptst, n_tst] = CTGFmap_ClassModel_Test(cls_id, mtd, ...
            exp_num, C_EXP_TEST, TestExpPlan, TestFeatVecs, ClassModel, ...
            TestClassIds, TestDocIds, TestFrmIds, fsuffix);
        iel = iel + 1;
        ElapsedTime(iel, C_ELAPSED_RANK)     = rnk_type;
        ElapsedTime(iel, C_ELAPSED_CLASS)    = cls_id;
        ...
        %********* release resources **********
        clearvars ClassModel Rank KeepVec ClassImp FeatVecs TrainFeatVecs TestFeatVecs FeatIds
        fclose('all');
